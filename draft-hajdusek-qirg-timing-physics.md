...
Consider two photons incident on a beamsplitter (BS) with reflectivity `dollarsignrdollarsign`.
We label the input modes by `dollarsignadollarsign` and `dollarsignbdollarsign`, and the output modes by `dollarsigncdollarsign` and `dollarsignddollarsign`.
We are interested in the observed behavior at the output modes of the BS.
There are four possible cases that may occur:

* Case A: photon in mode `dollarsignadollarsign` is reflected, while photon in mode `dollarsignbdollarsign` is transmitted.
...
~~~svg
{::include Figures/HOM.svg}
~~~
{: #fig-hom-interference title="HOM interference of two photons."}
...
where `dollarsigna ^{\dagger} _jdollarsign` and `dollarsignb ^{\dagger} _kdollarsign` are the bosonic creation operators corresponding to BS input modes `dollarsignadollarsign` and `dollarsignbdollarsign`, respectively.
The indices `dollarsignjdollarsign` and `dollarsignkdollarsign` represent other properties of the photons that determine how distinguishable the photons are.
For example, `dollarsignjdollarsign` and `dollarsignkdollarsign` could represent
...
The action of the BS on the input modes is represented by a unitary operator `dollarsignU_{ab}dollarsign`,
...
For a 50:50 BS when `dollarsignr=1/2dollarsign`, we obtain,
...
From this expression, we can see that when `dollarsignj=kdollarsign`, in other words when the input photons are indistinguishable, the output state has the following form,
...
In order to quantify the effect that distinguishability has on HOM interference, we consider the **probability of a coincidence detection**, `dollarsignp_{\text{co}}dollarsign`, where one photon is detected in the BS output mode `dollarsigncdollarsign`, and the other photon in output mode `dollarsignddollarsign`.
...
where `dollarsignP_idollarsign`, for `dollarsigni=c,ddollarsign`, are the projection operators representing a detection of a single photon in output mode `dollarsignidollarsign` of the BS.
For completely indistinguishable input photons that undergo the full HOM interference, we have `dollarsignp _{\text{co}}=0dollarsign`.
On the other hand, for fully distinguishable photons, the probability of a coincidence detection attains its maximum value `dollarsignp _{\text{co}}=1/2dollarsign`.

An often-used measure that quantifies the degree of HOM interference is the **visibility** `dollarsignVdollarsign`, defined via the probability of a coincidence detection,

~~~text
V = \frac{p _{\text{co}} ^{\text{max}} - p _{\text{co}} ^{\text{min}}}{p _{\text{co}}^{\text{max}}} = 1 - 2 p _{\text{co}} ^{\text{min}},
~~~

where we used the fact that the maximum probability of a coincidence detection is `dollarsign1/2dollarsign`.
We observe that the visibility varies from `dollarsignV=0dollarsign` for fully distinguishable input photons to `dollarsignV=1dollarsign` for perfectly indistinguishable ones.

Visibility `dollarsignVdollarsign` plays a useful role when modelling the effects of imperfect HOM interference in the context of entanglement swapping.
Consider the case when the input photons `dollarsignadollarsign`, `dollarsignbdollarsign` are entangled with auxiliary systems `dollarsigns_1dollarsign` and `dollarsigns_2dollarsign`, respectively.
The BSA performs ES by measuring the input photons, entangling systems `dollarsigns_1dollarsign` and `dollarsigns_2dollarsign` in the process.
Fidelity of the new entangled pair is directly proportional to the visibility `dollarsignVdollarsign` of the HOM interference.
Denote by `dollarsign\rho_{s_1s_2}^{\text{no-deph}}dollarsign` the density matrix resulting from an ideal ES at the BSA with unit visibility of the HOM interference.
...
where `dollarsign\rho_{s_1s_2}^{\text{deph}}dollarsign` is a fully dephased state obtained by setting all off-diagonal elements of `dollarsign\rho_{s_1s_2}^{\text{no-deph}}dollarsign` to zero.
...
The maximum probability of a coincidence detection is obtained for orthogonally polarized photons, for example when `dollarsignj=Hdollarsign` and `dollarsignk=Vdollarsign`.
Here, `dollarsignHdollarsign` denotes horizontal polarization and `dollarsignVdollarsign` denotes vertical polarization.
The output state of the two photons is
...
We can immediately see that `dollarsignp _{\text{co}} ^{\text{max}}=1/2dollarsign`.

In general, the two input photons will have polarizations given by two unit vectors, `dollarsignj=\epsilondollarsign` and `dollarsignk=\epsilon'dollarsign`.
...
The projection operators corresponding to a detection even at detector `dollarsignidollarsign` ($i=a,b$) are given by

~~~text
P _i = |1;\epsilon\rangle _i \langle 1;\epsilon| _i + |1;\epsilon'\rangle _i \langle 1;\epsilon'| _i.
~~~

Either an `dollarsign\epsilondollarsign`-polarized or an `dollarsign\epsilon'dollarsign`-polarized photon is detected in the output mode `dollarsignidollarsign`.
...
where the overlap between the polarization unit vectors is parametrized by `dollarsign\thetadollarsign`, and can be written as `dollarsign\langle\epsilon'|\epsilon\rangle = \cos\thetadollarsign`.
...
When the photons have identical polarization, `dollarsign\theta=0dollarsign`, the visibility reaches its maximum of `dollarsignV=1dollarsign`.
...
~~~
{: #fig-visibility-polarization title="Visibility of polarization mismatched photons."}
...
where `dollarsign\theta_1dollarsign` and `dollarsign\theta_2dollarsign` represent the polarization drift induced in the single-mode fiber.
Photons `dollarsigna_2dollarsign` and `dollarsignb_1dollarsign` are incident onto a BS, where they undergo HOM interference.
Following the same calculation as above, it can be shown that the probability of a coincidence event is `dollarsignp_{\text{co}} = 1/4dollarsign`, regardless of the polarization drift.
This suggests that the visibility is insensitive to the polarization drift.
However, the polarization drift must be tracked regardless because it affects the fidelity of the post-ES state of photons `dollarsigna_1dollarsign` and `dollarsignb_2dollarsign`,
...
The photon wave packet of a photon can be represented by its **spectral amplitude function** `dollarsign\phi(\omega)dollarsign`, where `dollarsign\int d\omega |\phi(\omega)|^2=1dollarsign`.
...
~~~
{: #fig-spectral-distinguishability title="Spectral distinguishability of two photons."}
...
Single-photon state with a spectral amplitude function `dollarsign\phi(\omega)dollarsign` is a superposition written as

~~~text
|1;\phi\rangle_a = \int d\omega \phi(\omega) a ^{\dagger}(\omega) |0\rangle_a,
~~~

where `dollarsigna ^{\dagger}(\omega)dollarsign` creates a photon in the BS input mode `dollarsignadollarsign` with frequency `dollarsign\omegadollarsign`.
Two input photons with arbitrary spectral functions `dollarsign\phidollarsign` and `dollarsign\varphidollarsign` are described by
...
The projection operators corresponding to a detection event in output mode `dollarsigncdollarsign` and output mode `dollarsignddollarsign` are given by
...
The form of this expression is the same as the one in subsection on polarization above, where the probability of a coincidence detection depended on the overlap between the polarization vectors `dollarsign\epsilondollarsign` and `dollarsign\epsilon'dollarsign`.
Now, `dollarsignp _{\text{co}}dollarsign` depends on the overlap between the spectral amplitude functions.
If the input photons are fully distinguishable, their respective spectral amplitude functions `dollarsign\phi(\omega)dollarsign` and `dollarsign\varphi(\omega)dollarsign` are orthogonal and the integrals vanish, meaning `dollarsignp _{\text{co}}=1/2dollarsign`.
On the other hand, for completely indistinguishable input photons we have `dollarsign\phi(\omega)=\varphi(\omega)dollarsign`, and due to the normalization condition we obtain `dollarsignp _{\text{co}}=0dollarsign`.
...
where the state of the photon is a mixture of pure single-photon states with spectral amplitude function `dollarsign\phi_k(\omega)dollarsign`, weighted by probability `dollarsignu_kdollarsign`.
The normalization condition requires that `dollarsign\sum_k u_k=1dollarsign`.
...
Due to the linearity of quantum mechanics, we can immediately write the expression for the probability of coincidence as a sum of pure-state coincidence probabilities weighted by `dollarsignu_kdollarsign` and `dollarsignv'_kdollarsign`,
...
We assume that the two spectral amplitude functions have the same standard deviation `dollarsign\sigma_a=\sigma_b=\sigmadollarsign`, which simplifies the expression for the probability of a coincidence detection to

~~~text
p _{\text{co}} = \frac{1}{2} \left( 1 - e^{-\frac{(\bar{\omega}_a-\bar{\omega}_b)^2}{2\sigma^2}} \right).
~~~

We observe that for identical photons, when `dollarsign\bar{\omega}_a=\bar{\omega}_bdollarsign`, the probability of a coincidence detection vanishes.
For fully distinguishable wave packets, when `dollarsign\bar{\omega}_a-\bar{\omega}_b\rightarrow\inftydollarsign`, the probability approaches 1/2, as expected.
We can now define visibility as a function of the difference between the central frequencies `dollarsign\bar{\omega}_a-\bar{\omega}_bdollarsign`,
...
The spectral amplitude functions have the same central frequencies, `dollarsign\omega_a=\omega_bdollarsign`, which gives the following expression for the probability of coincidence and visibility,
...
~~~
{: #fig-visibility-spectral title="Visibility for spectrally distinguishable photons."}
...
~~~
{: #fig-distinguishability-temporal title="Temporal distinguishability of two photons."}
...
Two input photons with arbitrary spectral functions `dollarsign\phidollarsign` and `dollarsign\varphidollarsign`, with photon `dollarsignbdollarsign` arriving late, are described by
...
Consider two identical pure Gaussian wavepackets that arrive at the BS with a time difference given by `dollarsign\taudollarsign`.
...
~~~
{: #fig-visibility-temporal title="Visibility of temporally distinguishable photons."}
...
* **Detection efficiency:** The overall probability that an incoming photon registers a count, denoted by `dollarsign\etadollarsign`.
This efficiency can be further broken down.
Probability of losing the photon before it reaches the detector is described by the _coupling efficiency_, `dollarsign\eta_{\text{coupling}}dollarsign`.
The type of material and geometry of the detector determine the photon _absorption efficiency_, `dollarsign\eta_{\text{absorption}}dollarsign`.
Finally, the probability that an electric signal is generated upon successful absorption of a photon is described by the _registering efficiency_, `dollarsign\eta_{\text{registering}}dollarsign`.
...
* **Recovery time:** Denoted by `dollarsign\tau_{\text{recovery}}dollarsign` and also known as 'dead' time.
...
~~~
{: #fig-timing-jitter title="Timing jitter for different detectors."}
...
In the previous Section, we used `dollarsign\taudollarsign` to denote the difference between the arrival time of the photons at the BSA.
...
Time of detection is in general different from the time of photon arrival due to finite time needed to generate the output signal described by the timing jitter `dollarsignJ_{\text{timing}}dollarsign`.
Therefore, we will use `dollarsign\taudollarsign` to denote the difference in detection time of the two photons.

Measurement at the BSA is successful when the correct pattern of detector clicks is observed, and the difference in detection times `dollarsign\taudollarsign` is smaller than a given detection **acceptance window**, `dollarsignT_{\text{window}}dollarsign`.
...
We refer to the minimum separation between the photons as the **separation time** `dollarsignT_{\text{separation}}dollarsign`.

The size of the separation time depends on the following:

* **Wave packet shape:** Individual photons cannot have overlapping spatial wavepackets, which may lead to incorrect assignement of entangled qubits following a successful meadurement at the BSA. We will use `dollarsignT_{\text{photon}}dollarsign` to denote the length of a wavepacket in seconds.
...
Here, the memory must be first initialized by cooling it to its ground state, a process which takes `dollarsign<1\text{ms}dollarsign`.
The memory is then excited by a laser pulse of approximately `dollarsign50\mu \text{s}dollarsign` that generates a photon.
...
SPDC is an inefficient process with success probability of around `dollarsign10^{-6}dollarsign` per pump photon. In system design, the intensity of the pump laser is adjusted so that the average number of photons is appropriate; generally this must be set below one photon per time window in order to avoid polluting the signal with two-photon states.
...
The separation time governs the maximum rate at which EPPS attempts to generate the entangled photon pairs, which is given by `dollarsign1/T_{\text{separation}}dollarsign`.
...
Observed detection pattern determined whether the post-measurement state of the remote quantum systems (either quantum memories or other photons) was `dollarsign|\Psi^+\rangledollarsign` or `dollarsign|\Psi^-\rangledollarsign`.
...
Consider a general state of the qubit, `dollarsign|\psi\rangle = \alpha |0\rangle + \beta |1\rangledollarsign`, where `dollarsign|\alpha|^2+|\beta|^2=1dollarsign`.
Measurement in an arbitrary basis `dollarsignMdollarsign` projects the initial state `dollarsign|\psi\rangledollarsign` onto one of the eigenvectors of `dollarsignMdollarsign`, given by `dollarsign\{|\phi\rangle,|\phi^{\perp}\rangle\}dollarsign`.
Probabilities of the two possible measurement outcomes are given by the overlaps between the initial state `dollarsign|\psi\rangledollarsign` and the eigenvectors of the observable `dollarsignMdollarsign`,
...
We read this notation as _probability of the measurement outcome being the state `dollarsign|\phi\rangledollarsign`, given that the initial state was `dollarsign|\psi\rangledollarsign`_.
For example, measurement in the Pauli `dollarsignZdollarsign` basis projects onto the states `dollarsign\{|0\rangle,|1\rangle\}dollarsign`, while measurement in the Pauli `dollarsignXdollarsign` basis projects onto the states `dollarsign\{|+\rangle,|-\rangle\}dollarsign`.

It is often difficult to directly measure the qubit in an arbitrary basis when it comes to real-world implementation.
In such a case, the qubit needs to be pre-rotated by an appropriate unitary operation, and then measured in the `dollarsignZdollarsign` basis, which can usually be implemented in a straightforward way.
This approach greatly simplifies the implementation of arbitrary measurements.

Consider that the observable `dollarsignMdollarsign` is related to the Pauli `dollarsignZdollarsign` by unitary `dollarsignUdollarsign`, `dollarsignM = U Z U ^{\dagger}dollarsign`.
This means the unitary `dollarsignUdollarsign` relates the eigenvectors of the two observables,

~~~text
|\phi\rangle = U |0\rangle, \quad\text{and}\quad |\phi^{\perp}\rangle = U |1\rangle.
~~~

We can perform measurement in the `dollarsignMdollarsign` basis by applying `dollarsignU ^{\dagger}dollarsign` to the initial state `dollarsign|\psi\rangledollarsign`, and them measuring it in the Pauli `dollarsignZdollarsign` basis.
...
~~~
{: #fig-measurement-1qubit title="Measurement of a single qubit."}

### Two-qubit measurements

The same principle of changing the measurement basis can be generalized to two qubits.
This time state `dollarsign|\psi\rangledollarsign` represents a general two-qubit state, unitary `dollarsignU ^{\dagger}dollarsign` acts on both qubits, which are both finally measured in Pauli `dollarsignZdollarsign` basis.
...
~~~
{: #fig-measurement-2qubit title="Measurement of two qubits."}
...
The second one is the internal degree of freedom, represented by the ground state `dollarsign|g\rangledollarsign` and the excited state `dollarsign|e\rangledollarsign`.
...
~~~
{: #fig-shelving title="Readout of trapped ions via shelving."}
...
If no fluorescence is observed, the ion is measured in the excited state `dollarsign|e\rangledollarsign`.
...
This approach was demonstrated experimentally using [$^{40}\text{Ca}^+dollarsign` ions](https://www.nature.com/articles/nature01494).
...
This led to high-fidelity demonstrations of `dollarsign>0.99dollarsign`, and gate times of around 50 microseconds.
...
Setting the HWP and QWP at particular angles applies the unitary `dollarsignU ^{\dagger}dollarsign` that picks the basis of the measurement, while the PBS filters out vertical and horizontal polarizations that then get detected by the detectors placed in the output paths of the PBS.
...
~~~
{: #fig-twodetector-polarization title="Measurement of polarization-encoded photons."}
...
~~~
{: #fig-polarization-table title="Table of polarization states."}
...
~~~
{: #fig-poincare-sphere title="Poincare sphere."}
...
~~~
{: #fig-waveplates-matrix title="Matrix representation of waveplates."}
...
~~~
{: #fig-waveplate-angles-table title="Table of polarization-basis measurement settings."}
...
The first approach is the _crossbar switch_ with all-to-all connectivity. Such an `dollarsignN\times Ndollarsign` switch can be reconfigured to accomodate all possible `dollarsignN!dollarsign` permutations of input-output pairs. One usual implementation of a crossbar switch is using **microelectromechanical systems (MEMS)** relying on small movable parts such as popup micromirrors, rotating prisms or [spinning holographic disks](https://onlinelibrary.wiley.com/doi/book/10.1002/0471213748). MEMS have usually low insertion loss and crosstalk, however due to their mechanical nature they suffer from slow switching times, which range from 10 microseconds to 10 miliseconds.
...
~~~
{: #fig-optical-switch title="Optical switch."}
...
~~~
{: #fig-time-delay title="Time delay introduced by an optical switch."}
...
In order to compensate for the propagation time delay, and ensure acceptable visibility at the BSAs, it is neccesary to adjust the path length of the photons with **optical delay lines** (ODLs). For small enough optical switches, it may be possible to characterize the propagation time delays for given photon pairs `dollarsign(X_i,X_j)dollarsign` assigned to a particular BSA prior to the opration of the switch. This would allow the ODLs to be set to precomputed configurations based on the connection request patterns. This approach will most likely not scale, at least in its general form, to larger optical switches.
